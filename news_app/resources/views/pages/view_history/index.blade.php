...
@section('content')
    ...
    <div id="page-wrapper">
        <div class="container-fluid" style="margin: 15px 5px">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 style="margin-bottom: 0">Đây là trang xem danh sách bài viết </h1>
                </div>
            </div>
            ...
            <div class="row" style="margin-top: 1rem; margin-bottom: 1rem">
                <div class="col-lg-6">
                    <a href={{ route('admin.posts.create') }} class="btn bg-success">Thêm Bài Viết</a>
                    <a href={{ route('admin.posts.index') }} class="btn bg-info"style="padding: 6px 2rem;">
                                        ...
                                        @else
                                            @foreach ($data_view as $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->content }}</td>
                                                    <td>{{ $item->view }}</td>
                                                    <th>{{ $item->id_catetory }}</th>
                                                    <td>{{ $item->created_at }}</td>
                                                    <td>{{ $item->updated_at }}</td>
                                                    <td>
                                                        <a href={{ route('admin.posts.edit', $item->id) }}><i
...
{{-- Link tới các file để xử lý phân trang --}}
@push('CSS')
